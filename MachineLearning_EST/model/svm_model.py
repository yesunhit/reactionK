...
from MachineLearning_EST.utils import function_utils
...
def PI_RF():
    ...
    for train_index, valid_index in kf.split(x_train_data):
        ...
        for i in range(train_count):
            model = SVR(kernel='poly', degree=5, tol=0.0001, gamma=0.01, coef0=1, C=100)
...
def PI_SVM():
    data = data_processing.load_data("../result/SVM_PI_score.xlsx")
    data = data.values
    filter_index = []
    for i in range(len(data)):
        if data[i][1] > 0:
...
function_utils.get_pi_data("../result/XGBoost_PI_score.xlsx", set_model())
PI_SVM()
